...
export const CMLogin: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const priDoc = await getDoc(doc(db, 'authorizedCM', email));
      if (!priDoc.exists()) {
        await auth.signOut();
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 selection:bg-primary selection:text-black">
      ...
      >
        <div className="flex flex-col items-center text-center mb-12">
          ...
          <h1 className="text-4xl font-black italic uppercase tracking-tighter text-white mb-4">Nexus.CM_Portal</h1>
          <p className="text-slate-500 font-black uppercase tracking-[0.3em] text-[10px] italic">Access strictly restricted to authorized Card Markers</p>
        </div>
